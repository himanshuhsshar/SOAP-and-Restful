...
public class AirTicketDao {
	...
	private final String SQL_INSERT_AIR_TICKET = "insert into airticket(pnr,airlines,name,source,destination,boarding_time,mobile_no,email_address,amount) values (?,?,?,?,?,?,?,?,?)";
	...
	public void saveAirTicket(final AirTicketBo airTicketBo) {
		jdbcTemplate.update(SQL_INSERT_AIR_TICKET, airTicketBo.getPnr(), airTicketBo.getName(),
				airTicketBo.getAirlines(), airTicketBo.getSource(), airTicketBo.getDestination(),
