...
/**
 * This class was generated by Apache CXF 3.5.3 2022-08-02T10:03:36.581+05:30
 * Generated source version: 3.5.3
 *
 */

@javax.jws.WebService(serviceName = "TicketManagerService", portName = "TicketManagerSOAPPort", targetNamespace = "http://www.goair.com/travel/service", endpointInterface = "com.goair.travel.service.TicketManager")
	...
	public com.goair.travel.types.AirTicket bookTicket(com.goair.travel.types.TravelPlan travelPlan) {
		...
		try {
			...
			for (Passenger passenger : travelPlan.getPassengers()) {
				passengerBo = PassengerBo.builder().fullname(passenger.getFullname()).age(passenger.getAge())
						.gender(passenger.getGender()).build();
				int passengerNo = passengerDao.savePassenger(passengerBo);
				passengerNos.add(passengerNo);
			}
			journeyInfo = travelPlan.getJourneyInfo();
			airTicketBo = AirTicketBo.builder().pnr(UUID.randomUUID().toString().substring(1, 6)).airlines("goair")
					.name(travelPlan.getPassengers().get(0).getFullname()).source(journeyInfo.getSource())
					.destination(journeyInfo.getDestination()).boardingTime(journeyInfo.getJourneyDt())
					.mobileNo(journeyInfo.getMobileNo()).emailAddress(journeyInfo.getEmailAddress()).amount(7937.00)
